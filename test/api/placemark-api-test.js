...
import { maggie, kerry, testPlacemarks } from "../fixtures.js";
...
suite("Placemark API tests", () => {
  ...
  test("create placemark", async () => {
    const returnedPlacemark = await placemarkService.createPlacemark(kerry);
    assert.isNotNull(returnedPlacemark);
    assertSubset(kerry, returnedPlacemark);
  });

  test("delete a placemark", async () => {
    const placemark = await placemarkService.createPlacemark(kerry);
